...
* `MIN(text)` 會選出長度最短的，`MAX(text)` 會選出長度最長的，如果長度相同時就依照字母順序。
```SQL
(
    SELECT
        MIN(CITY) AS CITY,
...
FROM (
    ...
    ON S.CHALLENGE_ID = SUB1.CHALLENGE_ID
...
```SQL
WITH CTE_1 AS (
    -- COUNT NUMBER OF SUBMISSION PER HACKER IN EACH DATE
    SELECT
        SUBMISSION_DATE,
        HACKER_ID,
        COUNT(*) AS CNT
...
CTE_2 AS (
    -- RANK THE NUMBER OF SUMISSION PER HACKER IN EACH DATE
    SELECT
        SUBMISSION_DATE,
        HACKER_ID,
        RANK() OVER (PARTITION BY SUBMISSION_DATE ORDER BY CNT DESC, HACKER_ID) AS RN
    FROM CTE_1
),
CTE_3 AS (
    -- GET THE HACKER ID AND NAME WHO HAS MOST SUBMISSION IN EACH DATE
    SELECT
    ...
    FROM CTE_2
    JOIN HACKERS
    ON CTE_2.HACKER_ID = HACKERS.HACKER_ID
    WHERE RN = 1
),
CTE_4 AS (
    -- ACCUMULATE COUNT NUM OF SUBMISSION PER HACKER IN EACH DATE
    -- CALCULATE NUMBER OF DAYS BETEWEN CURRENT SUBMISSION DATE AND FIRST DATE IN THE TABLE
    SELECT
        SUBMISSION_DATE,
        HACKER_ID,
        COUNT(HACKER_ID) OVER (PARTITION BY HACKER_ID ORDER BY SUBMISSION_DATE) AS CNT_SUBMISSIONS,
        DATEDIFF(DAY, (SELECT MIN(SUBMISSION_DATE) FROM SUBMISSIONS), SUBMISSION_DATE) + 1 AS CNT_DAYS
    FROM (
        SELECT
...
CTE_5 AS (
    -- IF CNT_SUBMISSIONS = CNT_DAYS, THEN THE HACKER SUBMITS EVERY DAY
    SELECT
        SUBMISSION_DATE,
        COUNT(SUBMISSION_DATE) AS NUM_SUBMISSIONS
    FROM CTE_4
    WHERE CNT_SUBMISSIONS = CNT_DAYS
    GROUP BY SUBMISSION_DATE
)
SELECT
    CTE_5.SUBMISSION_DATE,
    CTE_5.NUM_SUBMISSIONS,
    CTE_3.HACKER_ID,
    CTE_3.NAME
FROM CTE_5
JOIN CTE_3
ON CTE_5.SUBMISSION_DATE = CTE_3.SUBMISSION_DATE
```
